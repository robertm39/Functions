...
class SumImpl : public FunctionImpl {
private:
    Function f1;
    Function f2;
public:
    SumImpl(Function f1_, Function f2_): f1(f1_), f2(f2_) {}
    virtual b_float apply(b_float x) const {return f1.apply(x) + f2.apply(x);}
    virtual b_float nthDerivAtZero(int n) const {return f1.nthDerivAtZero(n) + f2.nthDerivAtZero(n);}
};
